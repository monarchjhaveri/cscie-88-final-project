...
public abstract class AbstractLineData implements LineData {
    ...
    public Map<String, Map<String, Object>> getIndexData() {
        ...
        System.out.println("getIndexData headers.length = " + headers.length);
        for (int column = offset; column < headers.length; column++) {
            System.out.println("getIndexData column = " + column);
    ...
    public Object getValue(int column) {
        System.out.println("getValue " + lineData[0].toString() + "column " + column);
        //return lineData[column];
        String[] x = lineData[0].toString().split(" ");
        System.out.println("LENGTH " + x.length);
        return x[column]; // [x.length - 1]; // column];
    }
